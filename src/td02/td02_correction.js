...
export const every = (predicate, array) => array.reduce(
  (acc, curr) => acc && predicate(curr),
  true // valeur initiale du prédicat
)
...
// La balise <img /> étant auto-fermante (`self-closing`), on se permettra de lui passer un tableau vide directement
...
export const generateMarkupForDog =
  (url, nom, description) =>
    figure(null, [
      img({src: url}),
      figcaption(null, [
...
export const generateMarkupForAllDogs = (dogs) =>
  createTag('div', null, dogs.map(dog => generateMarkupForDog(dog.url, dog.name, dog.description)))
...
export const errorMessage = createTag('div', null, [
  p(null, ['Vous aimez vraiment pas les chiens ? >:('])
])

export const conditionnallyDisplayDogs = (likesDogs, dogs) =>
  likesDogs && generateMarkupForAllDogs(dogs)
