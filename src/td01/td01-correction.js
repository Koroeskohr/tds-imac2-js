...
export var allTheAnimals = animals1.concat(animals2) // TODO
...
const number = 41
export const transformNumber = (n) => n + 1 // TODO

export const makeSomethingOutOfNumber = (f, n) => f(n) // TODO
...
export const generateArticleWithSignature = (body, sig, name) => body + sig(name) // TODO
export const finalArticle = generateArticleWithSignature(articleBody, sig3, 'Victor') // TODO
...
export const sampleAnimal = genAnimal('Lenin', 'bear', 4, 16, 'red')
 ...
 */
export const hensOnlyNameAndAge = (hens) => hens.map(({ name, age }) => ({ name, age }))
 ...
 */
export const mutatedHens = (hens) => hens.filter(({ legs }) => legs !== 2).map(x => x.name)
...
export const averageAgeForHens = (hens) => hens.reduce((acc, h) => acc + h.age, 0) / hens.length
...
export const max7CharsHens = (hens) => hens.filter(x => x.name.length <= 7).map(x => x.name)
...
export const specificHen = (hens) => hens.find(x => x.age > 15 && x.furColor === 'red')
